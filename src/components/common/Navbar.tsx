...
export const Navbar: React.FC<NavbarProps> = ({ onOpenLogin, onOpenRegister, onOpenUpgradeModal }) => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full backdrop-blur-xl bg-white/75 dark:bg-slate-900/80 border-b border-slate-200/60 dark:border-slate-800/80 transition-colors duration-200">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl px-4 pt-2 pb-6 space-y-2 animate-in fade-in slide-in-from-top-2">
          <button
            onClick={() => navigateTo('landing')}
          ...
          {!isPremium && onOpenUpgradeModal && (
            <button
              ...
              className="w-full text-left px-4 py-2.5 text-sm font-bold rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white"
            >
              Upgrade to Premium 👑
            </button>
          )}
